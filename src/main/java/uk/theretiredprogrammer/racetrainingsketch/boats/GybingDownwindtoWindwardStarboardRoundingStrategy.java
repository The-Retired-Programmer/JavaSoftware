 ...
 */
class GybingDownwindtoWindwardStarboardRoundingStrategy extends RoundingStrategy {

    private final double clearance;

    GybingDownwindtoWindwardStarboardRoundingStrategy(double clearance) {
    ...
    void nextTimeInterval(Decision decision, BoatElement boat, CourseLegWithStrategy leg, Angle winddirection) {
        ...
        if (leg.getEndLocation().angleto(boat.getLocation())
            ...
            decision.setMARKROUNDING(followingleg != null
                    ? winddirection.sub(boat.getClosehauled())
                    : boat.getDirection(),
                    CLOCKWISE);
        }
    }
