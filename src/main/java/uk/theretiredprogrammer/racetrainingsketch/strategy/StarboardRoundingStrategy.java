 ...
 */
class StarboardRoundingStrategy extends RoundingStrategy {

    private final Function<Angle, Angle> getDirectionAfterTurn;
    
    StarboardRoundingStrategy(BoatElement boat,
    ...
    final void nextTimeInterval(Decision decision, BoatElement boat, CourseLegWithStrategy leg, Angle winddirection) {
        ...
        if (leg.getEndLocation().angleto(boat.getLocation())
                .gteq(getOffsetAngle.apply(onPort, winddirection))) {
            decision.setMARKROUNDING(getDirectionAfterTurn.apply(winddirection), CLOCKWISE);
        }
    }
