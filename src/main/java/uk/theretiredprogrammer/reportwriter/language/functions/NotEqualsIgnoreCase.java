...
public class NotEqualsIgnoreCase extends BooleanExpression {
    ...
    public static void reduce(Language language, OperatorStack operatorstack, OperandStack operandstack) {
        ...
        operandstack.push(new NotEqualsIgnoreCase(DataTypes.isStringExpression(operandstack.pop()), rhs));
    }

    private final StringExpression lhs;
    private final StringExpression rhs;

    public NotEqualsIgnoreCase(StringExpression lhs, StringExpression rhs) {
        super("NOTEQUALSIGNORINGCASE expression");
        this.lhs = lhs;
        this.rhs = rhs;
    }
